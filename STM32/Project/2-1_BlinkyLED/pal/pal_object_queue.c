...
ErrorStatus PAL_ObjectQueue_Enqueue(PalObjectQueue_HandleTypeDef *hQueue, void *pElement)
{
	...
	// 附加功能：统计队列的占用率
	if(length > hQueue->MaxSize)
	{
		hQueue->MaxSize = length; 
	}
 ...
 * @返回值：占用率 0~100
 */
uint16_t PAL_ObjectQueue_GetOccupency(PalObjectQueue_HandleTypeDef *hQueue)
{
	return 100 * hQueue->MaxSize / hQueue->Capcity;
}
